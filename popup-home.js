	...
	function init() {
        if (localStorage.getItem('storedWords') != null) {
   		 ...
   		 $.each(wordList, function(key, value) {
			    var word = value;
			    $('<li/>').appendTo('#bankList ul').text(value).append('<input id="checkBox" type="checkbox">');
		 });
        }
    }
	init();
	...
	$('#addBtn').click(function(){
		 ...
		 if(!wordList.includes(inputValue)){
				...
				localStorage.setItem('storedWords', wordList.toString());
				 $('<li/>').appendTo('#bankList ul').text(inputValue).append('<input id="checkBox" type="checkbox">');
		 }
		 $("testArea").val() = "";
	});
